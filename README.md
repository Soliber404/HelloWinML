...
### [什么是Windows ML？](https://learn.microsoft.com/zh-cn/windows/ai/windows-ml/)

Windows ML是微软官方开发的高性能的可靠 API，用于在 Windows 设备上部署硬件加速的 ML 推理。

Windows ML于[2018年3月7日](https://www.51cto.com/article/567621.html)的Windows开发者日首次对外公布

优势：
1. 开发更轻松，WinML内置在系统中（Windows 10 和 Windows Server 2019）
2. 广泛的硬件支持（CPU、GPU、AI加速器），并保证各种受支持硬件的行为保持一致（bit一致？）
3. 低延迟、实时结果（与TensorRT相比如何？还需要进一步调研/实验验证）
4. 提高灵活性，降低运营成本（主要指本地部署，无需上云）

### 使用Windows ML API的前提/限制？

在[教程](https://learn.microsoft.com/zh-cn/windows/ai/windows-ml/api-reference)中有对教程运行条件的声明：
...
在[Windows 机器学习入门](https://learn.microsoft.com/zh-cn/windows/ai/windows-ml/get-started#in-box-vs-nuget-winml-solutions)中对比了内置和NuGet的解决方案，可见**最低可以支持的操作系统为Windows 8.1**
...
1. CPU
2. Default，由系统自动选择合适的设备
3. DirextX，使用 GPU 或AI加速器等其他 DirectX 设备
...
每次推理前，都需要更新绑定时对应的输入Feature对象？还是重新绑定(TODO: 待实验验证)
...
## WinML/ONNXRuntime/TensorRT性能对比调研/实验（TODO）
